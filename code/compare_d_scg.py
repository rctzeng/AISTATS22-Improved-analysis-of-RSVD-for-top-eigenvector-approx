...
opt = parse_arg()
T, MODE, ROUND, q = int(opt.T), 'adj', 'R', 1
np.random.seed(1234)
...
for dname in ['wikivot', 'referendum', 'slashdot', 'wikicon']:
    ...
    df = df.append({"d":-1, "dataset":dname, "algo":'eigsh', "R":1.0, "eigval":classic_eigval, "obj":classic_obj, "time":classic_time}, ignore_index=True)
    for d in [1,5,10,25,50]:
        ...
        for t in range(T):
            ...
            df = df.append({"d":d, "dataset":dname, "algo":'RSVD', "R":vs2[-1]/classic_eigval, "eigval":vs2[-1], "obj":ps2[-1], "time":ts2[-1]}, ignore_index=True)
            # RandSum
            ...
            df = df.append({"d":d, "dataset":dname, "algo":'RSum', "R":vs3[-1]/classic_eigval, "eigval":vs3[-1], "obj":ps3[-1], "time":ts3[-1]}, ignore_index=True)
        ...
        df.to_csv('SCG-d_q{}-{}.csv'.format(q, ROUND), index=False)
